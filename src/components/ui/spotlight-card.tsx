...
interface GlowCardProps {
  children: ReactNode;
  className?: string;
  glowColor?: 'brand' | 'blue' | 'purple' | 'green' | 'red' | 'orange';
  size?: 'sm' | 'md' | 'lg';
  width?: string | number;
...
const glowColorMap: Record<string, { base: number, spread: number, saturation?: number, lightness?: number }> = {
  brand: { base: 23, spread: 0, saturation: 67, lightness: 54 },
...
export const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  const getInlineStyles = () => {
    const baseStyles: React.CSSProperties & Record<string, any> = {
      ...
      '--lightness': lightness !== undefined ? lightness : 70,
      '--radius': '14',
      '--border': '1.5',
      ...
      backgroundImage: `radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 210) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 70) * 1%) / 0.2), transparent
      )`,
  ...
  const beforeAfterStyles = `
    ...
    [data-glow]::before {
      background-image: radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 210) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 50) * 1%) / 0.7), transparent 100%
      );
      filter: brightness(2);
    }
